...
class SoundSynth {
  ...
  final _cache = <SoundKind, Uint8List>{};
  ...
  Future<void> play(SoundKind kind) async {
    ...
    final v = s.soundVolume.clamp(0, 1).toDouble();
    if (v <= 0) return;
    final wav = _cache.putIfAbsent(kind, () => _renderWav(kind, v));
    try {
      await _player.stop();
      await _player.play(BytesSource(wav));
    } catch (e) {
      // audioplayers can complain on some headless platforms — non-fatal.
      if (kDebugMode) {
  ...
  Uint8List _renderWav(SoundKind kind, double volume) {
    final samples = switch (kind) {
      SoundKind.move => _woodClick(volume * 0.85, 220, 0.16),
      SoundKind.capture => _stack([
          _woodClick(volume * 1.0, 150, 0.22),
          _delayed(_woodClick(volume * 0.45, 320, 0.08), 0.018),
        ], 0.32),
      SoundKind.castle => _stack([
          _woodClick(volume * 0.9, 200, 0.18),
          _delayed(_woodClick(volume * 0.7, 260, 0.14), 0.07),
        ], 0.32),
      SoundKind.check =>
        _chord(volume * 0.55, const [660, 880, 1100], 0.32),
      SoundKind.promote => _shimmer(volume * 0.6),
      SoundKind.end => _chord(volume * 0.6, const [392, 523, 659, 784], 0.9),
    };
